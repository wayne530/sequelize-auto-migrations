...
let resUp =`{ fn: "createTable", params: [
    ...
    ${JSON.stringify(Object.assign(action.options, { transaction: null }))}
] }`;
            ...
            case 'dropTable':
            {
                let res = `{ fn: "dropTable", params: ["${action.tableName}", {"transaction": null}] }`;
                commandsUp.push(res);
...
let resUp = `{ fn: "addColumn", params: [
    ...
    { "transaction": null }
            ...
            case 'removeColumn':
            {
                let res = `{ fn: "removeColumn", params: ["${action.tableName}", "${(action.options && action.options.field) ? action.options.field : action.columnName}", {"transaction": null}] }`;
                commandsUp.push(res);
                ...
                function next() {
                    if (index < migrationCommands.length)
                    {
                        ...
                        for (let param of params) {
                            if (typeof(param) === 'object' && ('transaction' in param)) {
                                param['transaction'] = t;
                            }
                        }
...
`;
    ...
    name = name.replace(' ', '_');
    let filename = path.join(migrationsDir, getCurrentYYYYMMDDHHmms() + '-' +
        revision.toString().padStart(4, '0') + ((name != '') ? `-${name}` : '') + '.js');
